...
function unlock_partially_locked_balance()
{
  ...
  then
    test_fail "${FUNCNAME[0]}: The locked of account1 is incorrect, expected \"$locked_quantity $symbol\" but observed \"$account1_locked_before_lock\""
    return 1
  fi
  ...
  if [[ $? -ne 0 ]]
  then
    test_fail "The unlock failed although it should have succeeded: $result"
    return 1
  fi
  ...
  then
    test_fail "${FUNCNAME[0]}: The locked of account1 is incorrect, expected \"$quantity $symbol\" but observed \"$unlocking_fund_quantity\""
    return 1
  fi
...
function unlock_wrong_authority()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite the from account not being the authorizer"
    return 1
  fi
...
function unlock_wrong_receiver()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite the to account not existing"
    return 1
  fi
...
function unlock_wrong_quantity_amount()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite quantity being zero"
    return 1
  fi
  result=$( (cleos push action -f $chex_contract unlock "[$account1, $account2, \"-1.00000000 $symbol\", \"memo\"]" -p $account1) 2>&1)
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite quantity being negative"
    return 1
  fi
...
function unlock_wrong_quantity_symbol()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite symbol being incorrect"
    return 1
  fi
...
function unlock_no_locked_balance()
{
  ...
  if [[ $account1_balance_after_unlock != "0.00000000 $symbol" ]]
  then
    test_fail "${FUNCNAME[0]}: The balance of account1 is incorrect, expected \"0.00000000 $symbol\" but observed \"$account1_balance_after_unlock\""
    return 1
  fi

  local account2_balance_after_unlock=$(cleos get table $chex_contract $account2 accounts | jq -r .rows[0].balance)
  if [[ $account2_balance_after_unlock != "$quantity $symbol" ]]
  then
    test_fail "${FUNCNAME[0]}: The balance of account1 is incorrect, expected \"$quantity $symbol\" but observed \"$account2_balance_after_unlock\""
    return 1
  fi
...
function unlock_no_balance()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite account1 having no funds"
    return 1
  fi
...
function unlock_when_locked()
{
  ...
  if [[ $? -eq 0 ]]
  then
    test_fail "The unlock succeeded, despite account1 having its funds locked up"
    return 1
  fi
